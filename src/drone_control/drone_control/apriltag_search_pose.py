...
import tf2_ros
from drone_control.scripts.tag_transform_test import (
...
class AprilTagSearchNode(Node):
    ...
    def detection_cb(self, msg: AprilTagDetectionArray):
        ...
        for det in msg.detections:
            ...
            if match:
                ...
                cam_pose.header.frame_id = 'camera_link'  # Explicitly tie it to the drone
                
                world_pose = transform_to_world(
                    self, self.tf_buffer, cam_pose, world_frame=self.world_frame)
    ...
    def run(self):
        ...
        if not self.tag_found:
            ...
            self.get_logger().info('Handover to landing controller complete.')
